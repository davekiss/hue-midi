...
export interface MidiPCMessage {
  channel: number;
  program: number;  // 0-127 preset/program number
  timestamp: number;
}
...
export interface HueLight {
  ...
  productName?: string;
  modelId?: string;
  // true if the light is currently reachable via the Hue Bridge
  reachable?: boolean;
  capabilities: {
    color: boolean;
    brightness: boolean;
...
export interface LightState {
  ...
  transitionTime?: number; // in 100ms increments
  gradient?: Array<{ x: number; y: number }>; // For gradient lights
  gradientMode?: 'interpolated_palette' | 'interpolated_palette_mirrored' | 'random_pixelated' | 'segmented_palette';
}
...
export interface ChaseAnimationPresetParams {
  ...
  stopCount?: number;
  stepCount?: number;
  gradientMode?: LightState['gradientMode'];
}
...
export interface SceneLightState {
  targetId: string; // light or grouped light id
